...
- **rules/analyzers** apply the heuristics shipped in `browserforensics/core.py`.
- **score** ranks by severity.
- **MCP server** (`browserforensics mcp`) exposes `scan` for Cognis.Studio agents.
